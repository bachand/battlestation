...
public struct RandomNumber {
  ...
  init(_ allowedValues: CountableClosedRange<ValueType> = 0...9) throws {
  ...
  private let allowedValues: CountableClosedRange<ValueType>
  
  lazy var value: ValueType = {
    // This downcase is OK since we assert in the initalizer.
    let arc4random_upper_bound = UInt32(allowedValues.upperBound)
